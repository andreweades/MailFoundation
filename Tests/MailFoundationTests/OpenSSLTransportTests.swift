...
@Test("OpenSSL transport factory creation")
func openSSLTransportFactoryCreation() throws {
    let transport = try AsyncTransportFactory.make(host: "localhost", port: 1, backend: .openssl)
    #expect(transport is OpenSSLTransport)
}

@available(macOS 10.15, iOS 13.0, *)
@Test("OpenSSL transport conforms to AsyncStartTlsTransport")
func openSSLTransportStartTlsConformance() throws {
    let transport = try AsyncTransportFactory.make(host: "localhost", port: 1, backend: .openssl)
    #expect(transport is AsyncStartTlsTransport)
}
#endif

#if canImport(COpenSSL) && canImport(Security) && !os(iOS)
import Foundation
@preconcurrency import Security
import Darwin

@available(macOS 10.15, iOS 13.0, *)
@Test("OpenSSL transport STARTTLS integration success with validation disabled")
func openSSLTransportStartTlsSuccess() async throws {
    let identity = try await OpenSSLTestIdentityProvider.identity()
    let server = try OpenSSLTestServer(identity: identity)
    server.start()
    defer { server.stop() }

    let transport = OpenSSLTransport(host: "127.0.0.1", port: server.port)
...
@available(macOS 10.15, iOS 13.0, *)
@Test("OpenSSL transport STARTTLS integration fails with validation enabled")
func openSSLTransportStartTlsValidationFailure() async throws {
    let identity = try await OpenSSLTestIdentityProvider.identity()
    let server = try OpenSSLTestServer(identity: identity)
    server.start()
    defer { server.stop() }
    ...
    do {
        ...
        _ = try await startTlsTask.value
        #expect(Bool(false), "Expected certificate validation to fail")
    } catch {
        #expect(Bool(true))
    }
}

private enum OpenSSLTestError: Error {
    case identityImportFailed
    case socketFailed
}

@available(macOS 10.15, iOS 13.0, *)
private enum OpenSSLTestIdentityProvider {
    @MainActor
    static func identity() throws -> SecIdentity {
        ...
        guard let data = Data(base64Encoded: base64) else {
            throw OpenSSLTestError.identityImportFailed
        }
...
@available(macOS 10.15, iOS 13.0, *)
private final class OpenSSLTestServer: @unchecked Sendable {
    ...
    private func setupSocket() throws {
        ...
        guard listenFD >= 0 else {
            throw OpenSSLTestError.socketFailed
        }
